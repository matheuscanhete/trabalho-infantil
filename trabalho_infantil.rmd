...
valores <- colorRampPalette(brewer.pal(11, "Spectral"))(13)

ggplotly(
  dados %>%
  drop_na(.,atividade) %>% 
  ggplot(aes(atividade, V9058, fill = atividade)) +
  labs(y = "Horas habitualmente trabalhadas", x = "Atividade", title = "Horas trabalhadas por atividade") +
  geom_boxplot() +
...
```{r, echo = F}
#dados %>% plot_ly(x = ~nmr_irmaos, y = ~V4742, width = 1000, height = 450)

ggplotly(dados %>% filter(!V4742 > 999999999) %>% 
  ggplot(aes(filhos_domicilio, V4742, color = V4742)) + 
  labs(x = "filhos no domicílio", y = "rendimento domiciliar per capita", color = "R$") +
  scale_color_gradient(low = "#8c9dd1", high = "#041e6e") +
  geom_point(), width = 1000, height = 400)
```
